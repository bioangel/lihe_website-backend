...
public class ConnectTest {
    ...
    public void get_zhilian_login() throws Exception {
        ...
        HtmlPage indexPage = webClient.getPage(buildRequest(url));
        HtmlAnchor btn = indexPage.getAnchorByHref(loginUrl);
        HtmlPage loginPage = btn.click();
        for (int i = 0; i < 20; i++) {
            if (!loginPage.getByXPath("//div[@class='captcha-footer']").isEmpty()) {
                break;
            }
        ...
        WebClient webClientImg = new WebClient(BrowserVersion.CHROME);
        webClient.getCookieManager().getCookies().stream().forEach(item->{
            webClientImg.getCookieManager().addCookie(item);
        });

        Date data = new Date();
        WebRequest wq = buildRequest(captUrl + data.getTime());
        buildGetCaptRequest(wq, loginPage.getBaseURL().toString());
        UnexpectedPage img = webClientImg.getPage(wq);
        byte[] bytesImg = getPageToByte(img);
        writeBytesToFile(bytesImg);
        restoreZhiLianCaptcha();


        ScriptResult sr1 = loginPage.executeJavaScript("$.ajax;");
        System.out.print(sr1);

        ScriptResult sr2 = loginPage.executeJavaScript("$.ajax({\n" +
                "            url: \"/chk/verify\",\n" +
                "            type: \"post\",\n" +
                "            dataType: 'jsonp',\n" +
                "            jsonp: \"callback\",\n" +
                ...
                "            }\n" +
                "        });");
        System.out.print(sr2.getJavaScriptResult().toString());
    }
    ...
    public static void restoreZhiLianCaptcha() throws Exception {
        ...
        mergeImage(new String[]{System.getProperty("java.io.tmpdir") + "\\result1.jpg",
                ...
                System.getProperty("java.io.tmpdir") + "\\resultall.jpg");
        ...
        cutImage(System.getProperty("java.io.tmpdir") + "\\resultall.jpg",
                System.getProperty("java.io.tmpdir") + "\\result.jpg", 0, 0, 280, 130);

        cutImage(System.getProperty("java.io.tmpdir") + "\\headertmp.jpg",
                System.getProperty("java.io.tmpdir") + "\\header.jpg", 0, 45, 100, 40);
        for (int i = 0; i < col * 2; i++) {
            new File(System.getProperty("java.io.tmpdir") + "\\capt" + i + ".jpg").delete();
        }
        ...
        new File(System.getProperty("java.io.tmpdir") + "\\resultall.jpg").deleteOnExit();
    }
    ...
    private List<NameValuePair> getWebResponseHeader(HtmlPage page) {
